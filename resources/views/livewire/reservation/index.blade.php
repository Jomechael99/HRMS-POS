                            ...
                            <table class="w-full table-fixed border-collapse divide-y divide-gray-400 dark:divide-neutral-700 border border-gray-300 dark:border-neutral-600">
                                ...
                                @foreach($reservations as $data)
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 text-sm text-gray-800 dark:text-neutral-200">{{ $data->room->name }}</td>
                                        <td class="list-disc list-inside">
                                            @foreach($data->services as $service)
                                                <li>{{ $service->name }}</li>
                                            @endforeach
                                        </td>
                                        <td class="px-4 py-3 text-sm text-gray-800 dark:text-neutral-200">
                                            Check-in: {{ Carbon\Carbon::createFromFormat('Y-m-d', $data->check_in)->format('m-d-Y') }}
                                            <br>
                                            Check-out: {{ Carbon\Carbon::createFromFormat('Y-m-d', $data->check_out)->format('m-d-Y') }}
                                        </td>
                                        <td class="px-4 py-3 text-sm text-gray-800 dark:text-neutral-200">PHP {{ number_format($data->totalPrice, 2) }}</td>
