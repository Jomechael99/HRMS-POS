                        ...
                        <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-neutral-700">
                            ...
                            <div>
                                <div class="inline-flex gap-x-2">
                                    <a href="{{ route('services.create') }}" class="py-3 px-4 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none" aria-haspopup="dialog" aria-expanded="false" aria-controls="hs-slide-down-animation-modal" data-hs-overlay="#hs-slide-down-animation-modal">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full table-fixed border-collapse divide-y divide-gray-400 dark:divide-neutral-700 border border-gray-300 dark:border-neutral-600">
                                ...
                                @foreach($services as $data)
                                    <tr>
                                        <td class="px-4 py-3 text-sm text-gray-800 dark:text-neutral-200">{{ $data->id }}</td>
                                        <td class="px-4 py-3 text-sm text-gray-800 dark:text-neutral-200">{{ $data->name }}</td>
                                        <td class="px-4 py-3 text-sm text-gray-800 dark:text-neutral-200">
                                            @if($data->getFirstMediaUrl('images'))
                                                <img class="mx-auto" src="{{ asset('storage/' . $data->getFirstMedia('images')->id . '/' . $data->getFirstMedia('images')->file_name) }}" alt="Product Image" width="50" height="50">
                                            @else
                                                <img class="mx-auto" src="{{ asset('images/no-picture-available.png') }}" alt="No Picture Available" width="100" height="100">
                                            @endif
                                        </td>
                                        <td class="px-4 py-3 text-sm text-gray-800 dark:text-neutral-200">{{ $data->description }}
                                        <td class="px-4 py-3 text-sm text-gray-800 dark:text-neutral-200">PHP {{ number_format($data->price, 2) }}</td>
                                        <td class="px-4 py-3 text-sm text-gray-800 dark:text-neutral-200">{{ $data->created_at }}</td>
                                        <td class="px-1 py-3 text-center ">
                                            <a type="button" href="{{ route('services.edit', $data->id) }}" class="py-2 px-3 gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none">
                                                Edit
                                            </a>
                                            <button type="button" wire:click="delete({{ $data->id }})" class="py-2 px-3 gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-red-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none">
                                                Delete
                                            </button>
                                        </td>
                                    </tr>
